...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private apiKey: string | undefined = vscode.workspace
    .getConfiguration()
  ...
  private async saveThreadMessages(messages: any[]) {
      ...
      try {
        ...
        if (threadMessages?.data && threadMessages.data.length > 0) {
          ...
          const formattedMessages = reversedMessages.map((msg) => ({
            ...
            text: msg.content
              .filter(
                (contentItem): contentItem is MessageContentText =>
  ...
  private async createThread() {
    try {
      ...
      if (thread) {
        ...
        console.log(
          "Initialized new thread and saved thread ID",
          this.threadId
        );
      }
    } catch (error) {
  ...
  public async resolveWebviewView(
    ...
    const savedMessages = this.context.globalState.get<any[]>(
      "threadMessages",
      []
    );
    ...
    if (savedMessages.length > 0) {
      this._view.webview.postMessage({
        type: "restoreMessages",
        messages: savedMessages.map((msg: ThreadMessage) => ({
