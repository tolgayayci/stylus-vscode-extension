...
function exportAbiHandler(projectDataProvider, directProject, context) {
    (0, checkCargoStylus_1.checkCargoStylus)()
        .then(() => {
        const commandOptions = loadCommandOptions(context, "export-abi");
        selectProjectFolderAndExecuteExportAbi(projectDataProvider, commandOptions, directProject);
    })
        .catch((err) => {
...
function selectProjectFolderAndExecuteExportAbi(projectDataProvider, commandOptions, directProject) {
    if (directProject) {
        // Directly execute check for the provided project
        executeCargoStylusExportAbi(directProject.path, commandOptions);
        return;
    }
    ...
    if (projectsArray.length === 1) {
        ...
        executeCargoStylusExportAbi(projectsArray[0].folderPath, commandOptions);
    }
    else {
        ...
        vscode.window
            ...
            .then((selected) => {
            if (selected) {
                executeCargoStylusExportAbi(selected.folderPath, commandOptions);
            }
        });
    }
}
async function collectExportAbiOptionsAndExecute(folderPath, commandOptions) {
    let options = "";
...
function executeCargoStylusExportAbi(folderPath, commandOptions) {
    vscode.window
        ...
        .then((answer) => {
        if (answer === "Yes") {
            collectExportAbiOptionsAndExecute(folderPath, commandOptions);
        }
        else if (answer === "No") {
